...
import getAsyncSchemeThreshold from '@salesforce/apex/QuickAddPanelController.getAsyncSchemeThreshold';
...
const DELAY = 50;
const TIMEOUT_ASYNCSCHEME = 4000;
...
export default class QuickAddPanelcmp extends NavigationMixin(LightningElement) {
    ...
    @wire(getAsyncSchemeThreshold)
        asyncSchemeThreshold;
    ...
    async handleSuccess(event) {
        ...
        if(this.asyncSchemeThreshold?.data && this.asyncSchemeThreshold?.data != -1){
            setTimeout(() => {
                this.refreshStdComponents();
                this.fireForceRefreshEvent();
            }, TIMEOUT_ASYNCSCHEME);
